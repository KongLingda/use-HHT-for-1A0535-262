...
def create_gti(evt, time_col, binaryPar, freq, phase_bin, time_ref, inst):
    ...
    phase_bins, profile, profile_err = STpulse.fold_events(time[mk], *freq, nbin=32, gtis=gti, expocorr=True)
    plt.errorbar(np.hstack([phase_bins, phase_bins + 1]), np.hstack([profile, profile]), yerr=np.hstack([profile_err, profile_err]), drawstyle='steps-mid')
    plt.xlim(0, 2)
...
if __name__ == "__main__":
    ...
    ---------------
    python phase_resolved_spectroscopy.py outpath phase_0.1-0.2and0.7-0.8  -freq '1, 0.001'  -pipelineOutput P021400100101-20190120-01-01 -phase_bin_min 0.1 0.7 -phase_bin_max 0.2 0.8 -binPar_pb 9599040.0 -binPar_t0  53613 -binPar_asini 267 -binPar_e 0.47 -binPar_omega 2.27
    ---------------
        ...
        if os.path.exists(evt):
            ...
            if inst == "LE":
                ...
                if True:
                    ...
                    lebkgmap("evt_tmp.fits", "gti_tmp.fits", 'lebkgmap.tmp', 106, 1169, "%s/%s_LE_bkg" % (outpath, outfile))  # here I use le_blind, check with Mingyu
                ...
                if True:
                    ...
                    hebkgmap("evt_tmp.fits", ehkfile, 'gti_tmp.fits', hedead, 'he_pha_bkg_tmp.txt', 0, 256, "%s/%s_HE_bkg" % (outpath, outfile))
                    #os.system("rm evt_blind_tmp.fits")
                else:
